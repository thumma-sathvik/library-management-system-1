...
const AdminLayout = ({ children }) => {
  ...
  const [libraryInfo, setLibraryInfo] = useState({
    ...
    address: "Loading...",
    email: "Loading...",
    mobile: "Loading..." // Add mobile field
  });
  ...
  const fetchLibraryInfo = async () => {
    ...
    try {
      ...
      console.log('Library Info Response:', response.data);
      
      setLibraryInfo({
        name: response.data.name || 'Error loading',
        email: response.data.email || 'Error loading',
        address: response.data.address || 'Error loading',
        mobile: response.data.mobile || 'Error loading'
      });
      setError(null);
    } catch (error) {
                      ...
                      <div className="p-4 space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-700">
                            {libraryInfo.mobile?.replace(/(\d{3})(\d{3})(\d{4})/, '($1) $2-$3')}
                          </p>
                        </div>
                      </div>
                    )}
      ...
      >
        ...
        {!isMobileView && (
          <button
            ...
            className={`absolute -right-3 top-6 w-6 h-6 bg-white border border-gray-200 rounded-full shadow-sm items-center justify-center hover:bg-gray-100 transition-colors ${
              isMobileView ? 'hidden' : 'flex' // Use conditional rendering instead of conflicting classes
            }`}
          >
            {isSidebarCollapsed ? (
